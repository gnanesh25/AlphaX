...
import { Zap, Eye, EyeOff, ArrowRight, AlertCircle, Info, Sparkles } from 'lucide-react';
    ...
    try {
      ...
      if (error) {
        if (error.message.includes('not enabled') || error.message.includes('Unsupported provider')) {
          setInfoNotice(
            'Google OAuth provider is not yet enabled in your Supabase project dashboard (Auth > Providers > Google). You can use Demo Quick Login below to test immediately!'
          );
        } else {
          setErrorMsg(error.message);
        }
      ...
      if (msg.includes('not enabled') || msg.includes('Unsupported provider')) {
        setInfoNotice(
          'Google OAuth provider is not yet enabled in your Supabase project dashboard (Auth > Providers > Google). Use Quick Demo Login below!'
        );
      } else {
        setErrorMsg(msg);
      }
        ...
        {infoNotice && (
          ...
          >
            ...
            <span style={{ color: 'var(--text-secondary)', lineHeight: 1.5 }}>
              Google OAuth requires Client ID setup in your Supabase Dashboard under <strong>Auth &gt; Providers &gt; Google</strong>.
            </span>
          </div>
        )}
